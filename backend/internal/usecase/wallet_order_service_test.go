...
func TestWalletOrderService_CreateWithdrawInsufficient(t *testing.T) {
	...
	svc := usecase.NewWalletOrderService(repo, repo, repo, repo, repo, nil, repo)

	_, err := svc.CreateWithdraw(context.Background(), user.ID, usecase.WalletOrderCreateInput{Amount: 100000, Currency: "CNY"})
...
func TestWalletOrderService_ApproveRecharge(t *testing.T) {
	...
	order, err := svc.CreateRecharge(context.Background(), user.ID, usecase.WalletOrderCreateInput{Amount: 250000, Currency: "CNY"})
	if err != nil {
