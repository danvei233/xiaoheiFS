...
func TestCMSService_BasicFlow(t *testing.T) {
	...
	messageSvc := usecase.NewMessageCenterService(repo, repo)
	svc := usecase.NewCMSService(repo, repo, repo, messageSvc)
	...
	if _, _, err := svc.ListPosts(context.Background(), usecase.CMSPostFilter{Lang: "zh-CN", Limit: 10}); err != nil {
...
func TestReportService_OverviewAndSeries(t *testing.T) {
	...
	svc := usecase.NewReportService(repo, repo, repo)
...
func TestScheduledTaskService_ListAndUpdate(t *testing.T) {
	_, repo := testutil.NewTestDB(t, false)
	svc := usecase.NewScheduledTaskService(repo, nil, nil, nil, repo)
	...
	enabled := false
	interval := 600
	_, err := svc.UpdateTask(context.Background(), "vps_refresh", usecase.ScheduledTaskUpdate{
		Enabled:     &enabled,
