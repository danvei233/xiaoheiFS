...
func TestOrderService_SubmitPaymentRobotItems(t *testing.T) {
	...
	orderSvc := apporder.NewService(repo, repo, repo, repo, repo, repo, repo, repo, repo, nil, nil, robot, repo, repo, nil, repo, repo, repo, nil, nil, nil)

	if _, err := orderSvc.SubmitPayment(ctx, user.ID, order.ID, appshared.PaymentInput{
		Method:   "manual",
		Amount:   1000,
		Currency: "CNY",
		TradeNo:  "TRADE-1",
	...
	if _, total, err := orderSvc.ListOrders(ctx, appshared.OrderFilter{UserID: user.ID}, 10, 0); err != nil || total == 0 {
...
func TestAutomationLogContext(t *testing.T) {
	ctx := context.Background()
	if _, ok := appshared.GetAutomationLogContext(ctx); ok {
		t.Fatalf("expected no context")
	}
	ctx = appshared.WithAutomationLogContext(ctx, 10, 20)
	logCtx, ok := appshared.GetAutomationLogContext(ctx)
