...
func TestAdminService_UserOrderAndSettingsOps(t *testing.T) {
	...
	if _, total, err := svc.ListOrders(ctx, appshared.OrderFilter{}, 10, 0); err != nil || total == 0 {
	...
	if err := svc.DeleteOrder(ctx, admin.ID, approvedOrder.ID); err != appshared.ErrConflict {
...
func TestMessageCenterService_MarkAndNotify(t *testing.T) {
	repo := newTestRepo(t)
	ctx := context.Background()
	svc := appmessage.NewService(repo, repo)
