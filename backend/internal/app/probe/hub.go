...
var (
	ErrProbeOffline          = domain.ErrProbeOffline
	ErrProbeLogSessionClosed = domain.ErrProbeLogSessionClosed
)
...
func (h *Hub) RegisterConn(probeID int64, send func([]byte) error) {
	h.mu.Lock()
	defer h.mu.Unlock()
	h.conns[probeID] = &probeConnState{send: send, updatedAt: time.Now()}
}

func (h *Hub) UnregisterConn(probeID int64) {
	h.mu.Lock()
	defer h.mu.Unlock()
	delete(h.conns, probeID)
}

func (h *Hub) IsOnline(probeID int64) bool {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Hub) SendJSON(probeID int64, payload any) error {
	h.mu.RLock()
...
func (h *Hub) OpenLogSession(sessionID string, probeID int64, ttl time.Duration) {
	h.mu.Lock()
	defer h.mu.Unlock()
...
func (h *Hub) HasLogSession(sessionID string) bool {
	h.mu.RLock()
	defer h.mu.RUnlock()
...
func (h *Hub) SubscribeLogSession(sessionID string) (chan ProbeLogMessage, func(), error) {
	h.mu.Lock()
...
func (h *Hub) PublishLogChunk(sessionID, requestID, chunk string) {
...
func (h *Hub) PublishLogEnd(sessionID, requestID string) {
	h.publishLog(sessionID, ProbeLogMessage{
		Type:      "log_end",
		RequestID: requestID,
		At:        time.Now(),
...
func (h *Hub) publishLog(sessionID string, msg ProbeLogMessage) {
	h.mu.Lock()
	session := h.sessions[sessionID]
...
func (h *Hub) CloseLogSession(sessionID string) {
	h.mu.Lock()
	defer h.mu.Unlock()
	session := h.sessions[sessionID]
	if session == nil || session.closed {
