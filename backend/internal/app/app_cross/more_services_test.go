...
func TestCatalogService_Getters(t *testing.T) {
	...
	svc := appcatalog.NewService(repo, repo, repo)
...
func TestTicketService_Flow(t *testing.T) {
	...
	if _, _, err := svc.List(ctx, appshared.TicketFilter{UserID: &user.ID}); err != nil {
...
func TestAuthCartVPSAndStatus(t *testing.T) {
	...
	authSvc := appauth.NewService(repo, repo, repo)
	updated, err := authSvc.UpdateProfile(ctx, user.ID, appshared.UpdateProfileInput{Username: "profileu2", Email: "profileu2@example.com", QQ: "123"})
	if err != nil {
	...
	vpsSvc := appvps.NewService(repo, nil, repo)
	...
	statusSvc := appsystemstatus.NewService(provider)
...
func (p *fakeSystemProvider) Status(ctx context.Context) (appshared.ServerStatus, error) {
	return appshared.ServerStatus{Hostname: "host"}, nil
}
