...
func defaultSMSTemplates() []smsTemplateItem {
	...
	return []smsTemplateItem{
		...
		{ID: 5, Name: "phone_change_alert_old_contact", Content: "【XXX】安全提醒：您的账号手机号已于 {{time}} 从 {{old_phone}} 修改为 {{new_phone}}。如非本人操作，请立即修改密码并联系管理员。", Enabled: true, CreatedAt: now, UpdatedAt: now},
	}
}
...
func defaultEmailTemplates() []domain.EmailTemplate {
	return []domain.EmailTemplate{
		...
		{Name: "email_change_alert_old_contact", Subject: "邮箱变更安全提醒", Body: "您的账号邮箱已于 {{time}} 从 {{old_email}} 修改为 {{new_email}}。如非本人操作，请立即修改密码并检查账号安全。", Enabled: true},
	}
}
