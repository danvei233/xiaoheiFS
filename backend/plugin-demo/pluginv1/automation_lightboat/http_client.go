...
func (c *Client) CreateHost(ctx context.Context, req appshared.AutomationCreateHostRequest) (appshared.AutomationCreateHostResult, error) {
	...
	if resp.Code != 1 {
		return appshared.AutomationCreateHostResult{}, fmt.Errorf("automation error: %s", resp.Msg)
	}
	result := appshared.AutomationCreateHostResult{}
	if len(resp.Data) > 0 {
...
func (c *Client) GetHostInfo(ctx context.Context, hostID int64) (appshared.AutomationHostInfo, error) {
	...
	if resp.Code != 1 {
		if strings.Contains(resp.Msg, "创建中") {
			return appshared.AutomationHostInfo{HostID: hostID, State: 0}, nil
		}
		return appshared.AutomationHostInfo{}, fmt.Errorf("automation error: %s", resp.Msg)
	}
...
func (c *Client) ListHostSimple(ctx context.Context, searchTag string) ([]appshared.AutomationHostSimple, error) {
	...
	var out []appshared.AutomationHostSimple
	for _, item := range items {
		out = append(out, appshared.AutomationHostSimple{ID: item.ID, HostName: item.HostName, IP: item.IP})
	}
	return out, nil
}
...
func (c *Client) ListSnapshots(ctx context.Context, hostID int64) ([]appshared.AutomationSnapshot, error) {
	...
	out := make([]appshared.AutomationSnapshot, 0, len(items))
	for _, item := range items {
		out = append(out, appshared.AutomationSnapshot(item))
	}
	return out, nil
}
...
func (c *Client) ListBackups(ctx context.Context, hostID int64) ([]appshared.AutomationBackup, error) {
	...
	out := make([]appshared.AutomationBackup, 0, len(items))
	for _, item := range items {
		out = append(out, appshared.AutomationBackup(item))
	}
	return out, nil
}
...
func (c *Client) ListFirewallRules(ctx context.Context, hostID int64) ([]appshared.AutomationFirewallRule, error) {
	...
	out := make([]appshared.AutomationFirewallRule, 0, len(items))
	for _, item := range items {
		out = append(out, appshared.AutomationFirewallRule(item))
	}
	return out, nil
}
...
func (c *Client) ListPortMappings(ctx context.Context, hostID int64) ([]appshared.AutomationPortMapping, error) {
	...
	out := make([]appshared.AutomationPortMapping, 0, len(items))
	for _, item := range items {
		out = append(out, appshared.AutomationPortMapping(item))
	}
	return out, nil
}
...
func (c *Client) ListImages(ctx context.Context, lineID int64) ([]appshared.AutomationImage, error) {
	...
	var out []appshared.AutomationImage
	for _, item := range items {
...
func (c *Client) ListAreas(ctx context.Context) ([]appshared.AutomationArea, error) {
	...
	uniq := map[int64]appshared.AutomationArea{}
	for _, line := range lines {
		...
		uniq[line.AreaID] = appshared.AutomationArea{
			ID:    line.AreaID,
			Name:  fmt.Sprintf("Area %d", line.AreaID),
			State: line.State,
		}
	}
	out := make([]appshared.AutomationArea, 0, len(uniq))
...
func (c *Client) ListLines(ctx context.Context) ([]appshared.AutomationLine, error) {
	...
	out := make([]appshared.AutomationLine, 0, len(items))
	for _, item := range items {
		id := item.ID
		if id <= 0 {
			id = item.LineID
		...
		out = append(out, appshared.AutomationLine{ID: id, Name: name, AreaID: item.AreaID, State: item.State})
	}
	return out, nil
}
...
func (c *Client) ListProducts(ctx context.Context, lineID int64) ([]appshared.AutomationProduct, error) {
	...
	out := make([]appshared.AutomationProduct, 0, len(raw))
...
func (c *Client) GetMonitor(ctx context.Context, hostID int64) (appshared.AutomationMonitor, error) {
	...
	if resp.Code != 1 {
		return appshared.AutomationMonitor{}, fmt.Errorf("automation error: %s", resp.Msg)
	}
