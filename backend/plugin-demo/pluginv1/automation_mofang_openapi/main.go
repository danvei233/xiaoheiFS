...
type config struct {
	...
	ConfigOptionTmpl any    `json:"configoption_template"`
	CustomFieldTmpl  any    `json:"customfield_template"`
	OSTmpl           any    `json:"os_template"`
	TimeoutSec       int    `json:"timeout_sec"`
	Retry            int    `json:"retry"`
	DryRun           bool   `json:"dry_run"`
	EnableCatalog    bool   `json:"enable_catalog_sync"`
	EnableCreateHost bool   `json:"enable_create_host"`
}
...
func (s *coreServer) GetManifest(ctx context.Context, _ *pluginv1.Empty) (*pluginv1.Manifest, error) {
	...
	return &pluginv1.Manifest{
		PluginId:    "mofang_openapi",
		Name:        "Mofang OpenAPI Automation",
		Version:     "0.1.0",
		Description: "Mofang (ZJMF) OpenAPI automation plugin with JWT auth.",
		Automation: &pluginv1.AutomationCapability{
			...
			NotSupportedReasons: map[int32]string{
				int32(pluginv1.AutomationFeature_AUTOMATION_FEATURE_PORT_MAPPING): "mofang openapi has no unified NAT mapping interface",
				int32(pluginv1.AutomationFeature_AUTOMATION_FEATURE_BACKUP):       "mofang openapi has no unified backup interface",
				int32(pluginv1.AutomationFeature_AUTOMATION_FEATURE_SNAPSHOT):     "mofang openapi has no unified snapshot interface",
				int32(pluginv1.AutomationFeature_AUTOMATION_FEATURE_FIREWALL):     "mofang openapi has no unified firewall interface",
			},
		},
	}, nil
}
...
func (s *coreServer) GetConfigSchema(ctx context.Context, _ *pluginv1.Empty) (*pluginv1.ConfigSchema, error) {
	...
	return &pluginv1.ConfigSchema{
		JsonSchema: `{
  "title": "Mofang OpenAPI Automation",
  "type": "object",
  "properties": {
    "base_url": { "type": "string", "title": "Base URL", "description": "e.g. https://mofang.example.com" },
    "account": { "type": "string", "title": "OpenAPI Account", "description": "email or phone enabled for API login" },
    "api_password": { "type": "string", "title": "OpenAPI Password", "format": "password", "description": "used for /v1/login_api" },
    "product_id": { "type": "integer", "title": "Default Product ID", "description": "used by create-host flow", "default": 0, "minimum": 0 },
    "billing_cycle": { "type": "string", "title": "Default Billing Cycle", "default": "monthly" },
    "cancel_type": { "type": "string", "title": "Cancel Type", "default": "Immediate", "enum": ["Immediate", "Endofbilling"] },
    "cancel_reason": { "type": "string", "title": "Cancel Reason", "default": "automation cancel request" },
    "configoption_template": { "type": "object", "title": "ConfigOption Template", "description": "cart/products 的 configoption 模板，支持 {{cpu}}/{{memory_gb}}/{{disk_gb}}/{{bandwidth}}/{{host_name}}/{{sys_pwd}}" },
    "customfield_template": { "type": "object", "title": "CustomField Template", "description": "cart/products 的 customfield 模板，支持同上占位符" },
    "os_template": { "type": "object", "title": "OS Template", "description": "cart/products 的 os 模板，支持 {{os}}" },
    "enable_catalog_sync": { "type": "boolean", "title": "Enable Catalog Sync", "default": true },
    "enable_create_host": { "type": "boolean", "title": "Enable Create Host", "default": true },
  ...
  "required": ["base_url","account","api_password"]
}`,
		UiSchema: `{
  "api_password": { "ui:widget": "password" }
}`,
	}, nil
}
...
func (s *coreServer) ValidateConfig(ctx context.Context, req *pluginv1.ValidateConfigRequest) (*pluginv1.ValidateConfigResponse, error) {
	...
	if strings.TrimSpace(cfg.BaseURL) == "" || strings.TrimSpace(cfg.Account) == "" || strings.TrimSpace(cfg.APIPassword) == "" {
		return &pluginv1.ValidateConfigResponse{Ok: false, Error: "base_url/account/api_password required"}, nil
	}
...
func (s *coreServer) Init(ctx context.Context, req *pluginv1.InitRequest) (*pluginv1.InitResponse, error) {
	if strings.TrimSpace(req.GetConfigJson()) != "" {
		cfg, err := parseConfigWithDefaults(req.GetConfigJson())
		if err != nil {
...
func (s *coreServer) newClient() (*Client, error) {
	...
	return NewClient(
		...
		cfg.CancelType,
		cfg.CancelReason,
		cfg.ConfigOptionTmpl,
		cfg.CustomFieldTmpl,
		cfg.OSTmpl,
		cfg.EnableCatalog,
		cfg.EnableCreateHost,
		timeout,
	), nil
}
...
func parseConfigWithDefaults(raw string) (config, error) {
	var cfg config
	...
	if err := json.Unmarshal([]byte(raw), &present); err == nil {
		if _, ok := present["enable_catalog_sync"]; !ok {
			cfg.EnableCatalog = true
		}
		if _, ok := present["enable_create_host"]; !ok {
			cfg.EnableCreateHost = true
		}
	}
	return cfg, nil
}
...
func (a *automationServer) ListFirewallRules(ctx context.Context, req *pluginv1.ListFirewallRulesRequest) (*pluginv1.ListFirewallRulesResponse, error) {
	...
	for _, it := range items {
		out = append(out, &pluginv1.AutomationFirewallRule{
			...
			Ip:        toString(it["ip"]),
			Priority:  int32(toInt64(it["priority"])),
		})
	}
